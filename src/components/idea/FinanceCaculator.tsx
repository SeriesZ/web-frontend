...
const FinanceCaculator: React.FC<Props> = ({ itemData }) => {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const tableContainer = document.querySelector(".xScroll");

    if (tableContainer) {
      tableContainer.addEventListener("wheel", (event: Event) => {
        const wheelEvent = event as WheelEvent; // 확장된 타입으로 변환
        wheelEvent.preventDefault();
        tableContainer.scrollLeft += wheelEvent.deltaY; // deltaY 사용 가능
      });
    }
  });
  return (
            ...
            <tbody>
              ...
              <tr>
                ...
                {plan.map((yearData, index) => (
                  <td key={index}>
                    {Number(
                      yearData.advertisingCost.toFixed(0)
                    ).toLocaleString()}
                  </td>
                ))}
              </tr>
              ...
              <tr>
                ...
                {plan.map((yearData, index) => (
                  <th className={styled.total} key={index}>
                    {Number(
                      yearData.operatingIncome.toFixed(0)
                    ).toLocaleString()}
                  </th>
                ))}
              </tr>
